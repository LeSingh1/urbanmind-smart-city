...
export const FREMON_EQUITY_FIRST_METRICS: PlanningScores = {
  ...
  serviceGapCount: 1,
  totalEstimatedCost: 143_000_000,
}

export const FREMON_EXISTING_INFRASTRUCTURE: InfrastructureItem[] = [
  point('fremon-central-clinic', 'Fremon Civic Clinic', 'clinic', [-121.991, 37.548], 'Existing clinic is central but leaves south growth districts undercovered.', 0, 70, 0.82),
  point('fremon-north-bus-hub', 'North Loop Bus Hub', 'transit_stop', [-121.990, 37.586], 'Low-frequency transit anchor serving only part of the north district.', 0, 64, 0.78),
  point('fremon-east-school', 'Eastside Learning Center', 'school', [-121.948, 37.548], 'Existing education capacity east of downtown, below projected demand.', 0, 66, 0.8),
  point('fremon-central-park', 'Civic Commons Park', 'park', [-121.990, 37.554], 'Central open-space anchor, too small for 35% projected growth.', 0, 62, 0.77),
  point('fremon-fire-west', 'West Response Station', 'fire_station', [-122.038, 37.546], 'Fire response station near the west congestion corridor.', 0, 72, 0.84),
...
export const FREMON_UNDERSERVED_ZONES: UnderservedZone[] = [
  zone('fremon-north-transit-gap', 'North Transit Gap', 'transit_access', [37.586, -121.990], 1550, 0.84, 'North district has poor frequent-transit coverage as population grows.', 40, ['fremon-ai-north-transit']),
...
export const FREMON_GROWTH_PRESSURE_ZONES: GrowthPressureZone[] = [
  { id: 'fremon-growth-south', name: 'South District Housing Growth', center: [37.500, -121.990], radiusMeters: 1600, pressure: 'high', projectedGrowthPercent: 38, reason: 'New housing permits concentrate south of the civic core.' },
  ...
  { id: 'fremon-growth-north', name: 'North Transit Demand', center: [37.586, -121.990], radiusMeters: 1400, pressure: 'medium', projectedGrowthPercent: 28, reason: 'North district growth is transit constrained.' },
  { id: 'fremon-growth-west', name: 'West Commute Pressure', center: [37.548, -122.038], radiusMeters: 1550, pressure: 'medium', projectedGrowthPercent: 25, reason: 'Industrial and residential trips overlap on the arterial.' },
]

export const FREMON_DISTRICTS: DistrictProfile[] = [
  district('district-north', 'North Transit Gap', 'Poor transit access', 0.84, 31_000, 'Add frequent transit corridor', 40, 72, [37.586, -121.990], 'fremon-north-transit-gap'),
  ...
  district('district-south', 'South Emergency Gap', 'No nearby clinic', 0.92, 22_000, 'Add clinic and response coverage', 34, 79, [37.500, -121.990], 'fremon-south-emergency-gap'),
  district('district-central', 'Central Green Space Gap', 'Limited walkable green space', 0.74, 16_000, 'Add green corridor', 46, 74, [37.552, -121.990], 'fremon-central-green-gap'),
  district('district-west', 'West Congestion Zone', 'Congestion risk', 0.82, 9_000, 'Add bike and transit alternatives', 39, 62, [37.548, -122.038], 'fremon-west-congestion-zone'),
  district('district-innovation', 'Innovation District', 'Job growth pressure', 0.58, 12_000, 'Connect jobs to housing by transit', 59, 78, [37.565, -121.956]),
  district('district-housing', 'New Housing Expansion Zone', 'Housing needs services', 0.8, 20_000, 'Add mixed-use services and utilities', 43, 77, [37.512, -121.945], 'fremon-housing-utility-gap'),
...
export const FREMON_TOP_RECOMMENDATION: AIRecommendation = {
  ...
  title: 'Add a clinic in South Fremon',
  zoneName: 'South Emergency Gap',
  locationName: 'South Fremon Growth Zone',
  infrastructureType: 'clinic',
  coordinates: [-121.990, 37.500],
  reason: 'South Fremon has projected housing growth and weak emergency service coverage. A clinic here reduces emergency access gaps and improves equity.',
  expectedImpact: {
    emergencyAccess: 18,
    cityHealth: 5,
...
export const FREMON_PLACEMENT_SUGGESTIONS: PlacementSuggestion[] = [
  { id: 'suggest-clinic-south', rank: 1, title: 'South Fremon Growth Zone', category: 'clinic', coordinates: [-121.990, 37.500], expectedImpact: 'Emergency Access +18', costEstimate: 18_000_000, reason: 'Best clinic location: fills the highest-severity emergency access gap.', confidence: 0.87 },
  { id: 'suggest-clinic-housing', rank: 2, title: 'New Housing Expansion Edge', category: 'clinic', coordinates: [-121.948, 37.512], expectedImpact: 'Emergency Access +12', costEstimate: 16_000_000, reason: 'Serves new housing but is less central to the South Emergency Gap.', confidence: 0.78 },
  ...
  { id: 'suggest-school-housing', rank: 2, title: 'Housing Expansion School Site', category: 'school', coordinates: [-121.946, 37.522], expectedImpact: 'Education Access +15', costEstimate: 30_000_000, reason: 'Good site for new families, but farther from current east gap.', confidence: 0.76 },
...
export function getFremonBudgetSummary(level: BudgetLevel): BudgetSummary {
  ...
  const populationServed = level === 'low' ? 31_000 : level === 'medium' ? 65_000 : 108_000
